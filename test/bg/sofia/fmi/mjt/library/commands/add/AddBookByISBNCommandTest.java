...
public class AddBookByISBNCommandTest {
    @Mock
    ...
    @Test
    public void testAddBookByISBNCommandWithValidInput()
        ...
        Command command = new AddBookByISBNCommand(readerProfile, "9781840225563", booksClient);
        command.execute();
    ...
    @Test
    public void testAddBookByISBNCommandWithInvalidInput()
        throws IOException, InterruptedException {
        BooksClient booksClient = new BooksClient(mockHttpClient);
        when(mockBooksRequest.uri()).thenReturn(
            URI.create("https://www.googleapis.com/books/v1/volt"));

        String jsonResponse = "{\"kind\": \"books#volumes\",\"totalItems\": 0,\"items\": []}";
        ...
        ReaderProfile readerProfile = new ReaderProfile("Maria Ivanova", 20, "mimi03", "1234");
        Command command = new AddBookByISBNCommand(readerProfile, "", booksClient);
        assertThrows(LibraryException.class, command::execute
            , "The book with isbn 978184 was not found");
    }

    @Test
    public void testAddBookByISBNCommandWithInvalidRequestException()
        throws IOException, InterruptedException {
        BooksClient booksClient = new BooksClient(mockHttpClient);
        when(mockBooksRequest.uri()).thenReturn(
            URI.create(""));
